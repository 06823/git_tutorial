	...
	public partial class Form1 : Form
	{
		...
		internal static class Constants
		{
			...
			{	
				...
				/// </summary>
				internal const string OVERFLOW = "計算可能範囲を超えました";
		...
		private void OnDigitButton(string digit)
		{
			...
			//入力文字列を内部に保持
			decimal UserInputValue;
			if (decimal.TryParse(m_lastUserTypedRaw, NumberStyles.Any, CultureInfo.InvariantCulture, out UserInputValue))
			{
				m_displayValue = UserInputValue;
			}

            if (m_clearExpressionOnNextDigit)
            {
                textExpression.Text = "";
                m_clearExpressionOnNextDigit = false; 
            }
		...
		private void OnDotButton()
		{
			...
			decimal userInputValue;
			if (decimal.TryParse(stringToParse, NumberStyles.Any, CultureInfo.InvariantCulture, out userInputValue))
			{
				m_displayValue = userInputValue;
			}
		...
		private void OnOperatorButton(OperatorType op)
		{
			...
			try
			{
                ...
                //イコール直後の新しい演算子が入力された場合の処理
                if (StartNewChainAfterEqual(op))
				{
					return;
				}
                //パーセントキー入力直後に演算子が入力された場合の処理
		...
		{
			var currentExpressionText = (textExpression.Text == null ? "" : textExpression.Text).Trim();

            //上書きモードで、式が空でなく、かつ「=」で終わっていない場合のみ行う処理
            if (!(m_textOverwrite && currentExpressionText.Length > 0 && !currentExpressionText.EndsWith(Constants.Symbol.EQUAL)))
			{
				return false;
			}
			...
			{
				if (currentExpressionText.EndsWith(o))
				{
					textExpression.Text = currentExpressionText.Substring(0, currentExpressionText.Length - o.Length) + GetOperatorSymbol(op);
					m_currentOperatorType = op;
		...
		private bool HandlePercentThenOperator(OperatorType op)
		{
			...
			var currentInputValue = GetCurrentValue();
			PerformPendingCalculation(currentInputValue);
			if (IsError())
			{
				return true;
			}
		...
		private void OnPercentButton()
		{
            ...
            //演算子が未設定で、式が数値のみ、かつ直前の操作が％でない場合は 0 を表示して終了
            ...
            // ＝で計算を終えた直後に％が押された場合の処理
            if (ExpressionEndsWithEqual())
			{
				var rcurrentInputValue = GetCurrentValue();

				//加算減算処理のときはr×（r×0.01）を計算
				if (m_currentOperatorType == OperatorType.ADD || m_currentOperatorType == OperatorType.SUBTRACT)
				{
					m_percentChainFactor = rcurrentInputValue * Constants.Numeric.PERCENT_MULTIPLY; 
					var percentResultValue = rcurrentInputValue * m_percentChainFactor;   

					m_firstValue = percentResultValue;
					m_secondValue = Constants.Numeric.INITIAL_VALUE;
					m_currentOperatorType = OperatorType.NON;

					DisplayNumber(percentResultValue, true);
					textExpression.Text = FormatNumberForExpression(percentResultValue);
                ...
                {
					var percentResultValue = CalculatePercent(rcurrentInputValue);
            ...
            // ＝で計算を終えた直後に％が押され、かつ現在演算子が未設定のときの処理
            if (m_currentOperatorType == OperatorType.NON && m_inPercentChainAfterEqual && m_percentChainFactor != 0m)
			{
				var currepeatedPercentInput = GetCurrentValue();          
				var percentResultValue = currepeatedPercentInput * m_percentChainFactor;     
            ...
            //加算減算のときは左辺×％、乗算除算のときは％そのものを右辺に置換
            decimal adjustedRightOperand;
			if (m_currentOperatorType == OperatorType.ADD || m_currentOperatorType == OperatorType.SUBTRACT)
			{
				adjustedRightOperand = m_firstValue * percent;
			}
			else
			{
				adjustedRightOperand = percent;
			}

			textExpression.Text = string.Format("{0} {1} {2}",
				FormatNumberForExpression(m_firstValue),
				GetOperatorSymbol(m_currentOperatorType),
				FormatNumberForExpression(adjustedRightOperand));

			DisplayNumber(adjustedRightOperand, true);
		...
		private void OnClearEntryButton()
		{
            ...
            //直前に%気を入力して右辺を置き換えた状態でCEを押し場合、0に戻して式は維持
            ...
            // negate(result)で単独の結果を表示している状態でCEを押した場合、式も表示も0に戻す
		...
		private bool HasBinaryOperatorInExpression(string expr)
		{
			...
			int equalSignIndex = expr.LastIndexOf(Constants.Symbol.EQUAL);
			var body = (equalSignIndex >= 0) ? expr.Substring(0, equalSignIndex) : expr;
		...
		private void OnBackspaceButton()
		{
			...
			//イコールで計算が終わった直後に←が入力されると式をクリアして初期化
			if (ExpressionEndsWithEqual())
			{
				textExpression.Text = "";
				m_textOverwrite = true;
			...
			decimal currentNumericValue;
			if (decimal.TryParse(m_lastUserTypedRaw, NumberStyles.Any, CultureInfo.InvariantCulture, out currentNumericValue))
			{
				m_displayValue = currentNumericValue;
			}
		...
		private void OnToggleSignButton()
		{
            ...
            // イコール直後または negate表記中の場合値を反転させ途中計算式欄更新
            if (ExpressionEndsWithEqual() || isNegateExpr)
			{
				decimal negatedValue = -GetCurrentValue();
				DisplayNumber(negatedValue, true);
                ...
                {
                    m_firstValue = -Math.Abs(m_firstValue); 
                }

				m_clearExpressionOnNextDigit = true;
                m_preserveFormatOnToggle = false;
				m_lastActionWasPercent = false;
				m_lockRhsAfterAutoOp = true;   
				return;
			...
			{
				var leftOperand = m_firstValue;
				var negatedLeftOperand = -leftOperand;

				textExpression.Text = string.Format("{0} {1} {2}",
					FormatNumberForExpression(leftOperand),
					GetOperatorSymbol(m_currentOperatorType),
					string.Format("{0}({1})",
						Constants.SpecialDisplay.NEGATE_FUNCTION,
						FormatNumberForExpression(leftOperand)));

				m_displayValue = negatedLeftOperand;
				DisplayNumber(negatedLeftOperand, true);      
				m_preserveFormatOnToggle = false;
				m_lastActionWasPercent = false;
				m_lockRhsAfterAutoOp = true;    
				return;
		...
		private void StartNewNumber(string digit)
		{
			...
			decimal inputNumericValue;
			if (decimal.TryParse(stringToParse, NumberStyles.Any, CultureInfo.InvariantCulture, out inputNumericValue))
			{
				m_displayValue = inputNumericValue;
			}
		}
		...
		{
            //イコールが入力された直後、または演算子が未選択の場合は左辺を現在値に更新
		...
		private decimal ProcessEqualsLogic()
		{
			...
			var operatorSymbol = GetOperatorSymbol(m_currentOperatorType);
			var leftExpression = FormatNumberForExpression(left);
			var rightExpression = FormatNumberForExpression(right);

			var currentExpressionText = (textExpression.Text != null ? textExpression.Text.Trim() : string.Empty);

			if (!string.IsNullOrEmpty(currentExpressionText) &&
				!currentExpressionText.EndsWith(Constants.Symbol.EQUAL) &&
				currentExpressionText.StartsWith(Constants.SpecialDisplay.NEGATE_FUNCTION + "(", StringComparison.Ordinal))
				{
					if (currentExpressionText.EndsWith(operatorSymbol))
					{
						textExpression.Text = currentExpressionText + " " + rightExpression + " " + Constants.Symbol.EQUAL;
					}

					else
					{
						textExpression.Text = currentExpressionText + " " + Constants.Symbol.EQUAL;
					}
				}
			else
			{
				textExpression.Text = string.Format("{0} {1} {2} {3}",
					leftExpression, operatorSymbol, rightExpression, Constants.Symbol.EQUAL);
			}
        ...
        private void DisplayNumber(decimal value, bool overwrite)
		{
			...
			var rounded = RoundResult(value);
			formattedText = FormatNumberForDisplay(rounded);

			textResult.Text = formattedText;
			if (!IsExponentDisplay())
			{
		...
		{
			if (string.IsNullOrEmpty(raw) || raw == "-" || (raw == "0" && !numDot))
			{
				return raw;
			}
			...
			var decimalPointIndex = raw.IndexOf('.');
			var integerPart = decimalPointIndex >= 0 ? raw.Substring(0, decimalPointIndex) : raw;
			var fractionalPart = decimalPointIndex >= 0 ? raw.Substring(decimalPointIndex + 1) : "";

			decimal parsedIntegerValue;
			if (decimal.TryParse(integerPart, NumberStyles.Number, CultureInfo.InvariantCulture, out parsedIntegerValue))
			{
				var formattedIntegerPart = parsedIntegerValue.ToString("#,##0", CultureInfo.InvariantCulture);
				var formattedResultText = (decimalPointIndex >= 0) ? (formattedIntegerPart + "." + fractionalPart) : formattedIntegerPart;
				if (isNegative) formattedResultText = "-" + formattedResultText;
		...
		private static int DecimalBase10Exponent(decimal x)
		{
			...
			if (absoluteValue >= 1m)
			{
				var valueString = decimal.Truncate(absoluteValue).ToString(CultureInfo.InvariantCulture);
				return valueString.Length - 1;
			}
			else
			{
				var valueString = absoluteValue.ToString("0.#############################", CultureInfo.InvariantCulture);
				var decimalPointIndex = valueString.IndexOf('.');
				var leadingZeroCount = 0;
				for (int i = decimalPointIndex + 1; i < valueString.Length && valueString[i] == '0'; i++)
				{
					leadingZeroCount++;
				}
		...
		private static decimal RoundToSignificantDigits(decimal x, int n)
		{
			...
			var decimalExponent = DecimalBase10Exponent(x);        
			var roundingScale = n - 1 - decimalExponent;                   

			if (roundingScale < 0)
			{
				var scaleFactor = -roundingScale;
				return Math.Round(x / Pow10(scaleFactor), 0, MidpointRounding.AwayFromZero) * Pow10(scaleFactor);
			}
			else
			{
				var safeScale = roundingScale > 28 ? 28 : roundingScale;
		...
		{
			const int significantDigits = Constants.Numeric.EXP_SIGNIFICANT_DIGITS; // 16
			...
			var rounded = RoundToSignificantDigits(value, significantDigits);

			var decimalExponent = DecimalBase10Exponent(rounded);
			var mantissa = rounded / Pow10(decimalExponent);
			...
			var mantAbsTrunc = decimal.Truncate(Math.Abs(mantissa));
			if (Math.Abs(mantissa) == mantAbsTrunc)
			{
				mantissaString = (mantissa >= 0 ? "" : "-") + mantAbsTrunc.ToString("0", CultureInfo.InvariantCulture) + ".";
			}
			else
			{
				mantissaString = mantissa.ToString("0.#############################", CultureInfo.InvariantCulture).TrimEnd('0');
			}

			var exponentString = (decimalExponent >= 0 ? "+" : "") + decimalExponent.ToString(CultureInfo.InvariantCulture);
		...
		private string FormatNumberForDisplay(decimal value)
		{
            ...
            //小さい値は17桁以上の場合指数表記　それ以外は固定小数
            if (abs < Constants.Numeric.SCI_SMALL_THRESHOLD)
            {
                var rounded = RoundToSignificantDigits(value, Constants.Numeric.EXP_SIGNIFICANT_DIGITS);
                ...
                if (dot >= 0)
                {
                    ...
                    if (leadingZeros + sigDigits <= Constants.Numeric.MAX_DISPLAY_DIGITS)
                    {
                        return fixedStr;
                    }
                }
		...
		{
			var resultText = textResult.Text;
			return (resultText.IndexOf('e') >= 0 || resultText.IndexOf('E') >= 0);
		}
		...
		private void AutoFitResultFont()
		{
			...
			FontFamily fontFamily = textResult.Font.FontFamily;
			FontStyle fontStyle = textResult.Font.Style;

			while (fontSize > Constants.FontSize.MIN_LIMIT)
			{
				using (Font trialFont = new Font(fontFamily, fontSize, fontStyle))  
				{
					...
					if (trialTextSize.Width <= textResult.ClientSize.Width)
					{
						if (Math.Abs(textResult.Font.Size - fontSize) > Constants.FontSize.SIZE_EPSILON)
						{
							...
							textResult.Font = new Font(fontFamily, fontSize, fontStyle);
							oldFont.Dispose();
						}
						return;
					}
			...
			if (Math.Abs(textResult.Font.Size - Constants.FontSize.MIN_LIMIT) > Constants.FontSize.REDUCTION_STEP)
			{
				...
				textResult.Font = new Font(fontFamily, Constants.FontSize.MIN_LIMIT, fontStyle);
